...
class IntelligentAnalyzer:
    ...
    def analyze(self, state: GitLabAgentState) -> str:
        ...
        # Intentar generar insights usando InsightsEngine si hay un LLM disponible
        try:
            engine = InsightsEngine(llm_provider=self.llm_client)
            insights_dict = engine.generate_all_insights(state)
            # Serializar a texto para mantener el contrato de retorno como string
            insights_text = json.dumps(insights_dict, ensure_ascii=False, indent=2)
            ...
            try:
                ...
                logger.error(f"Error en fallback de análisis inteligente: {e2}")
                return f"Error en análisis: {str(e2)}"
...
class RepositoryAgent:
    ...
    def __init__(self,
        ...
        logger.warning(f"📍 File path output: {self.output_dir}")
        ...
        self.llm_analyzer = get_llm_analyzer(llm_provider=llm_client)
        ...
        logger.warning("RepositoryAgent inicializado correctamente")
    ...
    def _build_workflow(self) -> StateGraph:
                        ...
                        try:
                            ...
                            if branches.totalCount > 0:
                                ...
                                all_branches_dicts.append({
                                        ...
                                        "days_since_last_commit": None,  # Se calcularía en post-proceso
                                        "status": "N/A"  # GitHub no proporciona facilmente info de antigüedad
                                    })
                        except Exception as e:
        ...
        def maintenance_report_node(state: GitLabAgentState) -> GitLabAgentState:
            ...
            state["status"] = "maintenance_report_generated"
            state["messages"].append({"role": "system", "content": "Reportes mantenimiento generados"})
            logger.info("✅ Reportes mantenimiento generados")
            return state
        ...
        def generate_reports_node(state: GitLabAgentState) -> GitLabAgentState:
            ...
            state["messages"].append({"role": "system", "content": "Reportes finales generados"})
        ...
        def generate_executive_summary_node(state: GitLabAgentState) -> GitLabAgentState:
            if self.analysis_mode == AnalysisMode.INTELLIGENT and self.llm_client:
                logger.warning("🧠 Recomendaciones generados por LLM...")
                insights = self.llm_analyzer.generate_executive_summary(state)
                if state["insights"] == None:
                    state["insights"] = []
                state["insights"].append(insights)
                logger.info("✅ Recomendaciones completado")
            else:
                state["insights"] = "Análisis inteligente no habilitado"
        ...
        def generate_cleanup_recommendations_node(state: GitLabAgentState) -> GitLabAgentState:
            if self.analysis_mode == AnalysisMode.INTELLIGENT and self.llm_client:
                logger.warning("🧠 Genera un plan de limpieza priorizado...")
                insights = self.llm_analyzer.generate_cleanup_recommendations(state)
                state["insights"].append(insights)
                logger.info("✅ Plan de limpieza priorizado completado")
            else:
                state["insights"] = "Análisis inteligente no habilitado"
        ...
        workflow.add_node("generate_maintenance_report", maintenance_report_node)
        ...
        workflow.add_node("generate_cleanup_recommendations", generate_cleanup_recommendations_node)
        ...
        workflow.add_edge("analyze_artifacts", "generate_maintenance_report")
        workflow.add_edge("generate_maintenance_report", "generate_executive_summary")
        workflow.add_edge("generate_executive_summary", "generate_cleanup_recommendations")
        workflow.add_edge("generate_cleanup_recommendations", "generate_reports")
    ...
    def _save_reports(self, state: GitLabAgentState):
        ...
        if self.analysis_mode == AnalysisMode.INTELLIGENT and self.llm_client and state["insights"]:
            GitLabUtils.save_json(state['insights'],
                                  os.path.join(self.output_dir, "reporte_analisis_inteligente.json"))
    ...
    def _print_summary(self, state: GitLabAgentState):
        ...
        if self.analysis_mode == AnalysisMode.INTELLIGENT:
            insights = state.get("insights", [])

            if insights:
                print("\n💡 INSIGHTS:")
                # print(f"   Resumen ejecutivo: {insights.get('executive_summary')}")
                # print(f"   Plan de riesgos: {insights.get('risk_assessment')}")
                # print(f"   plan de mantenimiento: {insights.get('maintenance_plan')}")
                print(f"   💼 Resumen ejecutivo: ")
                print(f"   {insights[0].get('executive_summary')}")
                print(f"   Nivel de riesgo: {insights[0].get('risk_level')}")
                print(f"   Mayor riesgo: {insights[0].get('top_risks')}")
                print(f"   Salud: {insights[0].get('health_score')} / 100")
                print(f"   Recomendaciones: {insights[0].get('recommendations')} / 100")

                fase1 = insights[1].get('phase_1_delete_now')
                print(f"   🧹 Recomendaciones de limpieza: ")
                print(f"   {fase1.get('priority')} Fase 1 Borrar ahora: ")
                print(f"   justificación: {fase1.get('rationale')}")
                print(f"   ramas: {fase1.get('branches')}")
                print(f"   riesgos: {fase1.get('risks')}")
                print(f"   tiempo estimado: {fase1.get('estimated_time')}")

                fase2 = insights[1].get('phase_2_review')
                print(f"   {fase2.get('priority')} Fase 2 Revisar: ")
                print(f"   justificación: {fase2.get('rationale')}")
                print(f"   ramas: {fase2.get('branches')}")
                print(f"   riesgos: {fase2.get('risks')}")
                print(f"   tiempo estimado: {fase2.get('estimated_time')}")

                fase3 = insights[1].get('phase_3_archive')
                print(f"   {fase3.get('priority')} Fase 3 Archivar: ")
                print(f"   justificación: {fase3.get('rationale')}")
                print(f"   ramas: {fase3.get('branches')}")
                print(f"   riesgos: {fase3.get('risks')}")
                print(f"   tiempo estimado: {fase3.get('estimated_time')}")
