...
def load_analysis_data(output_dir: str) -> Optional[Dict[str, Any]]:
    ...
    # Leer catálogo de artefactos si existe
    ia_analysis_file = output_path / "reporte_analisis_inteligente.json"
    if ia_analysis_file.exists():
        try:
            with open(ia_analysis_file, 'r', encoding='utf-8') as f:
                ia_analysis_data = json.load(f)
                analysis_data["insights"] = ia_analysis_data
        except Exception as e:
            logger.error(f"Error reading artifacts file: {e}")
...
def analyze(req: GenerateRequest, background: BackgroundTasks):
    """
    ...
    # credenciales
    repository_url = req.analyzeInfo.repository_url or os.getenv("REPOSITORY_URL") or os.getenv("GITLAB_URL", "https://gitlab.com")
    token = req.analyzeInfo.token or os.getenv("TOKEN") or os.getenv("GITLAB_TOKEN")
    ...
    if not req.analyzeInfo.auto_discover_groups and not req.analyzeInfo.project_path:
        raise HTTPException(
            status_code=400, 
    ...
    out_dir = req.analyzeInfo.output_dir or str(OUTPUTS_DIR / job_id)

    # AnalysisMode
    mode = AnalysisMode.INTELLIGENT if req.analyzeInfo.analysis_mode.lower() == "intelligent" else AnalysisMode.BASIC
    ...
    background.add_task(
        ...
        token=token,
        project_path=req.analyzeInfo.project_path,
        auto_discover_groups=req.analyzeInfo.auto_discover_groups,
        active_days=req.analyzeInfo.active_days,
        stale_days=req.analyzeInfo.stale_days,
        artifact_days_soon=req.analyzeInfo.artifact_days_soon,
        max_pipelines=req.analyzeInfo.max_pipelines,
        output_dir=out_dir,
        analysis_mode=mode,
        azure_config=req,
    )
...
def job_insights(job_id: str, req: GenerateRequest):
    """
    ...
    try:
        llm = get_llm(job_id=job_id, req=req)
        analyzer = get_llm_analyzer(llm)
...
def job_recommendations(job_id: str, req: GenerateRequest):
    """
...
def branch_health_analysis(job_id: str, project_name: str, branch_name: str, req: GenerateRequest):
    """
...
def job_insights(job_id: str):
    """Devuelve insights y datos de análisis asociados a un job"""
    import json

    logger.info(f"🔎 job_insights solicitado para {job_id}")
    job = get_job(job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="Job no encontrado")
    ...
    output_dir = str(OUTPUTS_DIR / job_id)
    # Leer versiones y releases
    insights_file = output_dir / "reporte_analisis_inteligente.json"
        ...
        try:
            with open(insights_file, 'r', encoding='utf-8') as f:
                ...
                if isinstance(insights_data, dict):
                    ...
                    maintenance_plan = insights_data.get('maintenance_plan', [])
                    insights_data["executive_summary"] = executive_summary
                    insights_data["risk_assessment"] = risk_assessment
                    insights_data["maintenance_plan"] = maintenance_plan
        except Exception as e:
            logger.error(f"Error reading versions file: {e}")
...
def _run_analysis_task(
    ...
    try:
        ...
        llm = get_llm(job_id, azure_config)
        if(llm == None):
        ...
        agent = RepositoryAgent(
            ...
            llm_client=llm,  # conecta tu LLM si quieres usar modo intelligent
...
def get_llm(job_id, req):
    try:
        if req.bearer_token == None: 
            ...
            if req.azure_endpoint and req.azure_deployment:
                ...
                if req.asset_id:
                    logger.info(f"   Axet User ID: {req.asset_id}")
